...
class UserSubscribed extends Notification implements ShouldQueue
{
        ...
        $clickAction = 'OPEN_NOTIFICATIONS')
    {
        ...
        $this->body = ($body)
            ? $body
            : '{$this->subscribedUser->anchor_text} has started editing {$this->entry->anchor_text} entry from {$this->tableName}.';
    ...
    public function toArray($operator)
    {
        return [
            ...
            'subscribedUserId' => $this->subscribedUser->id,
            'tableName' => $this->tableName,
            'entryId' => $this->entry->id,
            'subject' => trans('emails.new_subscriber_subject'),
            'compiled_message' => str_replace(
                '{name}',
                $this->subscribedUser->anchor_text,
                str_replace(
                    '{entry_name}',
                    ...
                    str_replace(
                        '{table_name}',
                        $this->tableName,
                        trans('emails.new_subscriber_message')
                    )
                )
            )
        ];
